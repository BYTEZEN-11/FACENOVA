...
class RobertaClassifier(ClassifierBase):
    name = 'roberta-base'

    def __init__(self, use_transformer: bool=False):
        self.use_transformer = use_transformer
        self._fallback = HeuristicClassifier(name='roberta-heuristic', bias=2.0, sensitivity=1.05)
        self._model = None
    ...
    def _load_transformer(self):
        try:
            ...
            self._model = AutoModelForSequenceClassification.from_pretrained('roberta-base', num_labels=3)
        except Exception as exc:
            print(f'RobertaClassifier: transformer load failed: {exc}')
            self._model = None
